          ...
          {["all","in-progress","completed"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                filter === f ? "bg-gradient-to-r from-[#832388] to-[#F0772F] text-white" : th(dark,"bg-white/5 text-white/50 hover:text-white hover:bg-white/10","bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-800")
              }`}>
        ...
        {filtered.map(c => (
          <div key={c.id} className={`rounded-2xl border transition-all duration-300 overflow-hidden ${card}`}>
            ...
            <div className="p-5">
              ...
              <button className={`w-full py-2.5 rounded-xl text-sm font-medium transition flex items-center justify-center gap-2 ${
                c.progress === 100 ? "bg-green-500/20 text-green-400 hover:bg-green-500/30" : "bg-gradient-to-r from-[#832388] to-[#F0772F] text-white hover:opacity-90"
              }`}>
    ...
    <div className={`min-h-screen ${bg} ${sText} flex overflow-hidden transition-colors duration-300`} style={{fontFamily:"'DM Sans', system-ui, sans-serif"}}>
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-6 lg:p-8 overflow-y-auto">
          {activeNav === "home" && <HomePage firstName={firstName} overallProgress={overallProgress} dark={dark} />}
