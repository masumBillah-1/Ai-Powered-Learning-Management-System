...
interface ICourse {
  ...
  // ✅ DB schema: flat price fields (not pricing object)
  price?: number;
  ...
  rating?: number;
  category: string;
  // ✅ DB schema: thumbnail is primary
  thumbnail?: string;
...
const getCoverUrl = (course: ICourse): string => {
  ...
  // 3. coverImage string
  if (typeof course.coverImage === "string" && course.coverImage.trim()) {
    return course.coverImage.trim();
  }
  return "";
};
...
export default function AdminCoursesPage() {
  ...
  // ✅ Get enrollment count from either field
  const getEnrolledCount = (c: ICourse): number => c.enrollmentCount ?? c.enrolledCount ?? 0;
                    ...
                    <tr key={c._id} className="hover border-b border-base-200 last:border-0">
                      ...
                      <td>
                        <div className="flex items-center gap-1 text-sm font-bold opacity-60">
                          <Users size={12} />{getEnrolledCount(c)}
                        </div>
                      </td>
                  ...
                  <div key={c._id}
                    ...
                    <div className="p-4 flex flex-col flex-1 gap-3">
                      ...
                      <div className="flex items-center gap-3 pt-1 border-t border-base-200 text-xs">
                        <div className="flex items-center gap-1 opacity-60"><Users size={11} /><span className="font-bold">{getEnrolledCount(c)}</span></div>
                        {(c.rating || 0) > 0 && (
                          <div className="flex items-center gap-1">
