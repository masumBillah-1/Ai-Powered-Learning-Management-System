...
// ─── Toast styles (InstructorCoursesPage এর মতো) ─────────────────────────────
const tErr = {
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function getStatus(item: Announcement): 'Published' | 'Draft' {
  return (item.status || "Draft") as 'Published' | 'Draft';
}
...
// ─── StatusBadge — click করলে Draft ↔ Published toggle ──────────────────────
function StatusBadge({
  announcementId, currentStatus, theme, onToggled,
}: {
  announcementId: string;
  currentStatus: 'Published' | 'Draft';
  theme: string;
  onToggled: (id: string, newStatus: 'Published' | 'Draft') => void;
}) {
  ...
  const handleToggle = async (e: React.MouseEvent) => {
    ...
    const tid = toast.loading(
      newStatus === 'Published' ? '🚀 Publishing...' : '📝 Draft এ নিচ্ছে...',
      tLoading,
    );
    ...
    try {
      ...
      toast.success(
        newStatus === 'Published' ? '✅ Published হয়েছে!' : '📝 Draft এ সরানো হয়েছে',
        { id: tid, ...tOk },
      );
    } catch (err: any) {
      toast.error(`❌ ${err.message || "Status change করতে সমস্যা হয়েছে।"}`, { id: tid, ...tErr });
    } finally {
      setToggling(false);
    }
  };
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold
                 cursor-pointer hover:opacity-75 transition-all disabled:opacity-50 border-0"
      style={{
    ...
    >
      {toggling
        ...
        : <span className="w-2 h-2 rounded-full flex-shrink-0"
          style={{ backgroundColor: isPublished ? '#00C48C' : '#F89B29' }} />
      }
...
function AnnouncementFormFields({
  form, theme, courses, coursesLoading, onChange,
}: {
    ...
    <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
      ...
      <div className="form-control">
        <label className="label py-1">
          <span className="label-text text-xs font-bold uppercase tracking-wider opacity-50 flex items-center gap-1.5">
        ...
        <input type="text" placeholder="e.g. Assignment Due Date Reminder..."
          ...
          maxLength={200} className="input input-bordered w-full transition-all"
          style={{ borderColor: form.title ? "#832388" : undefined, boxShadow: form.title ? "0 0 0 3px rgba(131,35,136,0.1)" : undefined }}
        />
      </div>

      {/* Course — MongoDB থেকে instructor এর courses */}
      <div className="form-control">
        <label className="label py-1">
          <span className="label-text text-xs font-bold uppercase tracking-wider opacity-50 flex items-center gap-1.5">
          ...
          {coursesLoading && (
            <span className="label-text-alt flex items-center gap-1 text-xs opacity-40">
        ...
        >
          <option value="">
            {coursesLoading ? "Courses load হচ্ছে..." : "Select a course..."}
          </option>
          {courses.map(c => (
          ...
          {!coursesLoading && courses.length === 0 && (
            <option disabled>কোনো course পাওয়া যায়নি</option>
          )}
        </select>
      </div>
      ...
      <div className="form-control">
        ...
        <textarea placeholder="Announcement এর বিস্তারিত লিখুন..."
          ...
          maxLength={1000} rows={4} className="textarea textarea-bordered w-full resize-none transition-all"
...
// ─── Create Modal ─────────────────────────────────────────────────────────────
function CreateAnnouncementModal({ isOpen, onClose, theme, onSuccess, courses, coursesLoading }: {
  isOpen: boolean; onClose: () => void; theme: string;
  onSuccess?: (data: Announcement) => void;
  courses: ICourse[]; coursesLoading: boolean;
}) {
  ...
  const handleClose = () => {
    if (loading) return;
    setForm({ title: "", courseId: "", description: "", priority: "medium" });
    onClose();
  };

  const handleSubmit = async (publishNow: boolean) => {
    // Validation — modal এর ভেতরে inline error দেখাও
    ...
    setLoading(true);
    const tid = toast.loading(
      publishNow ? '🚀 Publishing...' : '💾 Draft সেভ হচ্ছে...',
      tLoading,
    );
    ...
    try {
      ...
      toast.success(
        publishNow ? '✅ Announcement Published!' : '📝 Draft সেভ হয়েছে',
        { id: tid, ...tOk },
      );
      onSuccess?.(data.notification);
      handleClose();
    } catch (err: any) {
      toast.error(`❌ ${err.message || "কিছু একটা সমস্যা হয়েছে।"}`, { id: tid, ...tErr });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal modal-open" style={{ zIndex: 9999 }}>
      ...
      <div className="modal-box relative w-full max-w-lg p-0 overflow-hidden rounded-2xl shadow-2xl"
        style={{ border: "1px solid rgba(131,35,136,0.25)", zIndex: 10000 }}>
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-base-300">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl shadow-lg"
              style={{ background: "linear-gradient(135deg, #832388, #E3436B)" }}>📢</div>
            <div>
              <h3 className="text-lg font-bold leading-tight">New Announcement</h3>
              <p className="text-xs opacity-40 mt-0.5">notifications collection এ save হবে</p>
            </div>
          </div>
          <button onClick={handleClose} disabled={loading}
            className="btn btn-ghost btn-sm btn-circle opacity-50 hover:opacity-100 cursor-pointer">
            <X size={18} />
          </button>
        </div>

        <AnnouncementFormFields
          form={form} theme={theme} courses={courses}
          coursesLoading={coursesLoading} onChange={handleChange}
        />

        {/* Footer */}
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-base-300"
          style={{ backgroundColor: theme === "dark" ? "rgba(131,35,136,0.05)" : "#fdf8ff" }}>
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold"
            style={{ backgroundColor: theme === "dark" ? sel.darkBg : sel.bg, color: sel.color }}>
            <sel.Icon size={12} /> {sel.label}
          </div>
          <div className="flex items-center gap-2 ml-auto">
            <button className="btn btn-ghost btn-sm cursor-pointer" onClick={handleClose} disabled={loading}>
              বাতিল
            </button>
            <button className="btn btn-sm gap-1.5 cursor-pointer border-0"
              style={{ backgroundColor: "rgba(131,35,136,0.12)", color: "#832388" }}
              onClick={() => handleSubmit(false)} disabled={loading}>
              {loading ? <span className="loading loading-spinner loading-xs" /> : <FileText size={13} />}
              Draft
            </button>
            <button className="btn btn-sm gap-1.5 text-white border-0 cursor-pointer"
              style={{
                background: loading ? "#ccc" : "linear-gradient(135deg, #832388, #E3436B, #F89B29)",
                boxShadow: loading ? "none" : "0 4px 12px rgba(131,35,136,0.35)",
              }}
              onClick={() => handleSubmit(true)} disabled={loading}>
              {loading ? <span className="loading loading-spinner loading-xs" /> : <Send size={13} />}
              {loading ? "হচ্ছে..." : "Publish"}
            </button>
          </div>
        </div>
      </div>
...
// ─── Edit Modal ───────────────────────────────────────────────────────────────
function EditAnnouncementModal({ item, theme, onClose, onSuccess, courses, coursesLoading }: {
  ...
  const handleSubmit = async (publishNow: boolean) => {
    ...
    try {
      ...
      toast.success(
        publishNow ? '✅ Published হয়েছে!' : '📝 Draft হিসেবে সেভ হয়েছে',
        { id: tid, ...tOk },
      );
      onSuccess?.(data.notification);
      handleClose();
    } catch (err: any) {
      toast.error(`❌ ${err.message || "Update করতে সমস্যা হয়েছে।"}`, { id: tid, ...tErr });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal modal-open" style={{ zIndex: 9999 }}>
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" onClick={handleClose} />
      <div className="modal-box relative w-full max-w-lg p-0 overflow-hidden rounded-2xl shadow-2xl"
        style={{ border: "1px solid rgba(227,67,107,0.25)", zIndex: 10000 }}>
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-base-300">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center text-xl shadow-lg"
              style={{ background: "linear-gradient(135deg, #E3436B, #F89B29)" }}>✏️</div>
            <div>
              <h3 className="text-lg font-bold leading-tight">Edit Announcement</h3>
              <div className="flex items-center gap-2 mt-0.5">
                <p className="text-xs opacity-40">পরিবর্তন করুন</p>
                {/* current status badge — header এ দেখাচ্ছে */}
                <span className="text-xs font-bold px-2 py-0.5 rounded-full"
                  style={{
                    backgroundColor: currentStatus === 'Published'
                      ? (theme === 'dark' ? '#0f2520' : '#d1fae5')
                      : (theme === 'dark' ? '#2a1f15' : '#fef3c7'),
        ...
        {/* Footer */}
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-base-300"
          style={{ backgroundColor: theme === "dark" ? "rgba(227,67,107,0.05)" : "#fff9f9" }}>
          <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold"
            style={{ backgroundColor: theme === "dark" ? sel.darkBg : sel.bg, color: sel.color }}>
            <sel.Icon size={12} /> {sel.label}
          </div>
          <div className="flex items-center gap-2 ml-auto">
            ...
            {/* Draft হিসেবে save */}
            <button className="btn btn-sm gap-1.5 cursor-pointer border-0"
              style={{ backgroundColor: "rgba(227,67,107,0.12)", color: "#E3436B" }}
              onClick={() => handleSubmit(false)} disabled={loading}>
              {loading ? <span className="loading loading-spinner loading-xs" /> : <FileText size={13} />}
              Draft সেভ
            </button>
            ...
            <button className="btn btn-sm gap-1.5 text-white border-0 cursor-pointer"
              style={{
                background: loading ? "#ccc" : "linear-gradient(135deg, #E3436B, #F89B29, #832388)",
                boxShadow: loading ? "none" : "0 4px 12px rgba(227,67,107,0.35)",
              }}
              onClick={() => handleSubmit(true)} disabled={loading}>
              {loading ? <span className="loading loading-spinner loading-xs" /> : <Send size={13} />}
              {loading ? "হচ্ছে..." : "Publish"}
...
export default function AnnouncementsPage() {
  ...
  // Instructor এর courses MongoDB থেকে fetch
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      const url = instructorId
        ? `/api/courses?instructorId=${instructorId}&limit=100`
      ...
      setCourses((data.courses || []).map((c: any) => ({
        _id: c._id, title: c.title, status: c.status,
      })));
    } catch (err: any) {
  ...
  const fetchAnnouncements = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/notifications?type=announcement&limit=50&all=true", { credentials: "include" });
      const data = await res.json();
  ...
  // Handlers
  const handleCreated = (n: Announcement) => { if (n?._id) setAnnouncements(p => [n, ...p]); else fetchAnnouncements(); };
  const handleEdited = (u: Announcement) => setAnnouncements(p => p.map(a => a._id === u._id ? u : a));
  const handleStatusToggled = (id: string, newStatus: 'Published' | 'Draft') =>
    setAnnouncements(p => p.map(a => a._id === id ? { ...a, status: newStatus } : a));
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.error(`❌ ${err.message || "Delete করতে সমস্যা হয়েছে।"}`, { id: tid, ...tErr });
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      {/*
        ── Toaster — navbar এর ঠিক নিচে দেখাবে ──────────────────────────────────
        containerStyle: top: 80 → navbar height (adjust করো প্রয়োজনে)
        right: 24 → right edge থেকে gap
      */}
      <Toaster
        position="top-right"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
        {[
          ...
          { label: "Drafts", value: counts.draft, color: '#F89B29', bLight: '#fef3c7', bDark: '#2a1f15', Icon: Edit2 },
        ].map(card => (
          <div key={card.label} className="card bg-base-100 shadow-lg border"
            style={{ borderColor: theme === 'dark' ? card.bDark : card.bLight }}>
            <div className="card-body p-5">
              <div className="flex items-start justify-between">
                ...
                <div className="w-12 h-12 rounded-xl flex items-center justify-center"
                  style={{ backgroundColor: theme === 'dark' ? card.bDark : card.bLight }}>
        ...
        <div className="card-body p-6 md:p-8">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search announcements or courses..."
          ...
          {fetchLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <button className="btn btn-sm cursor-pointer"
                ...
                onClick={fetchAnnouncements}>আবার চেষ্টা করুন</button>
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20">
              <div className="text-6xl mb-4">📢</div>
              <h3 className="text-xl font-bold mb-2">
                {announcements.length === 0 ? "কোনো Announcement নেই" : "কিছু পাওয়া যায়নি"}
              </h3>
              <p className="opacity-60 mb-6">
                {announcements.length === 0 ? "প্রথম announcement তৈরি করুন" : "Search বা filter পরিবর্তন করুন"}
              </p>
              {announcements.length === 0
                ? <button className="btn gap-2 cursor-pointer"
                  style={{ backgroundColor: '#832388', color: 'white', border: 'none' }}
                  onClick={() => setShowCreateModal(true)}>
                  <Plus size={16} /> প্রথম Announcement তৈরি করুন
                </button>
                : <button className="btn gap-2 cursor-pointer"
                  style={{ backgroundColor: '#832388', color: 'white', border: 'none' }}
                ...
                <thead>
                  <tr>
                    ...
                    <th className="text-center text-xs font-bold uppercase tracking-wider opacity-60">
                      Status
                      <span className="ml-1 opacity-40 normal-case font-normal text-[10px]">(click to toggle)</span>
                    </th>
                    ...
                    return (
                      <tr key={item._id} className="hover">
                        ...
                        <td>
                          ...
                          {courseTitle && <p className="text-xs opacity-60 italic">📚 {courseTitle}</p>}
                        ...
                        <td>
                          <div className="flex justify-end gap-2">
                            <button className="btn btn-ghost btn-sm cursor-pointer" title="View">
                              <Eye size={16} />
                            </button>
                            <button className="btn btn-ghost btn-sm cursor-pointer" title="Edit"
                              onClick={() => setEditItem(item)}>
                              <Edit2 size={16} />
                            </button>
                            <button className="btn btn-ghost btn-sm text-error cursor-pointer" title="Delete"
          ...
          <div className="p-6">
            ...
            {deleteItem && (
              <div className="rounded-xl p-4 mb-4 border"
                style={{ backgroundColor: theme === 'dark' ? '#2a1515' : '#fef2f2', borderColor: '#fca5a5' }}>
                <p className="text-sm font-bold text-error line-clamp-2">{deleteItem.title}</p>
                {getCourseTitle(deleteItem.courseId) && (
                  <p className="text-xs opacity-60 mt-1">📚 {getCourseTitle(deleteItem.courseId)}</p>
                )}
              </div>
            )}
            <div className="flex justify-end gap-3">
              <form method="dialog">
                <button className="btn btn-ghost cursor-pointer" onClick={() => setDeleteItem(null)}>
                  বাতিল
                </button>
              </form>
              <button className="btn text-white border-0 cursor-pointer gap-2"
      ...
      <CreateAnnouncementModal
        ...
        theme={theme} onSuccess={handleCreated}
