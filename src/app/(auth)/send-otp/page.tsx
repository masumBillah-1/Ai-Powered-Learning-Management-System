...
export default function SendOtpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // email টা query param এ পাঠাও verify page এ
      router.push(`/verify-otp?email=${encodeURIComponent(email)}`);
    } catch (err: any) {
      toast.error(err.message || "OTP send করা যায়নি");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="relative z-10 w-full max-w-md">
          <div className="bg-white/90 dark:bg-[#120B1E] border border-gray-200 dark:border-[#2D2438] rounded-2xl p-8 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">OTP Verification</h2>
              <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">
                আপনার email এ ৬ সংখ্যার OTP পাঠানো হবে।
              </p>
            </div>
            ...
            <div className="mt-5 text-center space-y-2">
              ...
              <Link href="/forgot-password" className="text-gray-500 dark:text-gray-400 hover:underline text-sm block">
                Email link দিয়ে reset করবেন? →
              </Link>
            </div>
          </div>
        </div>
